...
class FileUpload {
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        if (!$this->fileName) {
            $this->setErrorMsg('非法文件名或没有文件上传');
            return false;
        }

        if ($this->fileSize == 0) {
            $this->setErrorMsg('文件为空');
            return false;
        }

        if ($this->fileSize > $this->sizeLimit) {
            $this->setErrorMsg('上传文件大小超出限制');
            return false;
        }
        ...
        if (!file_exists($this->uploadDir)) {
            $this->setErrorMsg('上传文件夹不存在');
            return false;

        } else if (!is_writable($this->uploadDir)) {
            $this->setErrorMsg('上传文件夹不可写');
            return false;
        }
